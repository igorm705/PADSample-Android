...
class StickerPackValidator {
     ...
     */
    static void verifyStickerPackValidity( @NonNull StickerPack stickerPack, byte[] m_bytes)
            throws IllegalStateException {

        Log.i("RecyclerView", "class StickerPackValidator, " +
                " static void verifyStickerPackValidity( @NonNull StickerPack stickerPack, byte[] m_bytes)  ");
        ...
        if (!TextUtils.isEmpty(stickerPack.iosAppStoreLink) && !isValidWebsiteUrl(stickerPack.iosAppStoreLink)) {
            throw new IllegalStateException("Make sure to include http or https in url links, ios stickers_module store link is not a valid url: " + stickerPack.iosAppStoreLink);
        }
        if (!TextUtils.isEmpty(stickerPack.iosAppStoreLink) && !isURLInCorrectDomain(stickerPack.iosAppStoreLink, APPLE_STORE_DOMAIN)) {
            throw new IllegalStateException("iOS stickers_module store link should use stickers_module store domain: " + APPLE_STORE_DOMAIN);
        }
        ...
        try {
            if (bytes.length > TRAY_IMAGE_FILE_SIZE_MAX_KB * KB_IN_BYTES) {
    ...
    private static void validateSticker(@NonNull final String identifier,
        ...
        validateStickerFile(identifier, sticker.imageFileName, animatedStickerPack);
    }

    private static void validateStickerFile(@NonNull String identifier,
        ...
        try {
            if (!animatedStickerPack && bytes.length > STATIC_STICKER_FILE_LIMIT_KB * KB_IN_BYTES) {
                throw new IllegalStateException("static sticker should be less than " + STATIC_STICKER_FILE_LIMIT_KB + "KB, current file is " + bytes.length / KB_IN_BYTES + " KB, sticker pack identifier: " + identifier + ", filename: " + fileName);
            }
            if (animatedStickerPack && bytes.length > ANIMATED_STICKER_FILE_LIMIT_KB * KB_IN_BYTES) {
                throw new IllegalStateException("animated sticker should be less than " + ANIMATED_STICKER_FILE_LIMIT_KB + "KB, current file is " + bytes.length / KB_IN_BYTES + " KB, sticker pack identifier: " + identifier + ", filename: " + fileName);
            }
            try {
    ...
    private static void checkFrameDurationsForAnimatedSticker(@NonNull final int[] frameDurations,
                                                              @NonNull final String identifier, @NonNull final String fileName) {
